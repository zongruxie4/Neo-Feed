...
@UiThread
open class VkLibActionsListView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        val attr =
            context.obtainStyledAttributes(attrs, R.styleable.VkLibActionsListView, defStyleAttr, 0)
        val dHeight = attr.getDimensionPixelSize(
            R.styleable.VkLibActionsListView_vklib_alv_dividerHeight,
            a(1)
        )
        val dSize =
            attr.getDimensionPixelSize(R.styleable.VkLibActionsListView_vklib_alv_dividerSize, a(1))
        val dColor =
            attr.getColor(R.styleable.VkLibActionsListView_vklib_alv_dividerColor, -16777216)
        ...
        val paddingStart = attr.getDimensionPixelSize(
            R.styleable.VkLibActionsListView_vklib_alv_optionPaddingStart,
            0
        )
        val paddingEnd = attr.getDimensionPixelSize(
            R.styleable.VkLibActionsListView_vklib_alv_optionPaddingEnd,
            0
        )
        val iconSpace = attr.getDimensionPixelSize(
            R.styleable.VkLibActionsListView_vklib_alv_optionIconLabelSpace,
            0
        )
        val iconTint = Integer.valueOf(
            attr.getColor(
                R.styleable.VkLibActionsListView_vklib_alv_optionIconTint,
                -16777216
            )
        )
        val textSize = attr.getDimensionPixelSize(
            R.styleable.VkLibActionsListView_vklib_alv_optionLabelTextSize,
            (((getDisplayMetrics().scaledDensity * 16.0f) + 0.5f).toInt())
        )
        val textColor = attr.getColor(
            R.styleable.VkLibActionsListView_vklib_alv_optionLabelTextColor,
            -16777216
        )
        this.b =
            ActionStyle(oBg, paddingStart, paddingEnd, iconSpace, iconTint, textSize, textColor)
        attr.recycle()
