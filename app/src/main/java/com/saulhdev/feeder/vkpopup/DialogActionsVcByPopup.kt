...
class DialogActionsVcByPopup(private val d: View) {
    ...
    fun a(
        list: List<PopupItem>,
        setListViewParams: ((Pair<View, VkLibActionsListView>) -> Unit)? = null,
        onClick: (PopupItem) -> Unit
    ) {
        if (!a()) {
            ...
            val dialogActionsListView =
                inflate.findViewById<VkLibActionsListView>(R.id.dialog_actions_list_content)
